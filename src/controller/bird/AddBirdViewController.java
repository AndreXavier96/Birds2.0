...
import controller.brood.ViewAddBirdToBroodController;
import domains.Bird;
...
public class AddBirdViewController implements Initializable {
	...
	@FXML
	private Label LabelAlert, LbImagePath, LbBroodChose;
	@FXML
	private AnchorPane ApBuyPrice, ApBrood;
	@FXML
	private ImageView ImImage;
	@FXML
	private Button btnUpload, btnAddToBrood;
	...
	private Brood selectedBrood = null;
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
			...
			if (newValue != null) {
				...
				CbMutation.setConverter(new StringConverter<Mutation>() {
					...
					public Mutation fromString(String s) {
						return CbMutation.getItems().stream().filter(b -> b.getName().equals(s)).findFirst()
								.orElse(null);
					}
				});
				...
				TfFather.focusedProperty()
						.addListener((observableFocusFather, oldValueFocusFather, newValueFocusFather) -> {
							if (!newValueFocusFather) {
								LvFathers.setVisible(false); // Hide the ListView when TfFather loses focus
							}
						});
				LvFathers.focusedProperty()
						.addListener((observableFocusLvFathers, oldValueFocusLvFathers, newValueFocusLvFathers) -> {
							if (!newValueFocusLvFathers) {
								LvFathers.setVisible(false); // Hide the ListView when LvFathers loses focus
							}
						});

				TfMother.focusedProperty()
						.addListener((observableFocusMother, oldValueFocusMother, newValueFocusMother) -> {
							if (!newValueFocusMother) {
								LvMothers.setVisible(false); // Hide the ListView when TfFather loses focus
							}
						});
				LvMothers.focusedProperty()
						.addListener((observableFocusLvMothers, oldValueFocusLvMothers, newValueFocusLvMothers) -> {
							if (!newValueFocusLvMothers) {
								LvMothers.setVisible(false); // Hide the ListView when LvFathers loses focus
							}
						});

				LvFathers.getSelectionModel().selectedItemProperty().addListener(
						(observableSelectionLvFathers, oldValueSelectionLvFathers, newValueSelectionLvFathers) -> {
				...
				LvMothers.getSelectionModel().selectedItemProperty().addListener(
						(observableSelectionLvMothers, oldValueSelectionLvMothers, newValueSelectionLvMothers) -> {
		...
		TfFather.textProperty().addListener((observable, oldValue, newValue) -> {
			checkIfBroodCanBeAdded();
		});

		TfMother.textProperty().addListener((observable, oldValue, newValue) -> {
			checkIfBroodCanBeAdded();
		});
		DfDataEntrada.valueProperty().addListener((observable, oldValue, newValue) -> {
	        checkIfBroodCanBeAdded();
	    });
	}

	private void checkIfBroodCanBeAdded() {
		String fatherText = TfFather.getText();
		String motherText = TfMother.getText();
		Date selectedDate = Date.from(DfDataEntrada.getValue().atStartOfDay(ZoneId.systemDefault()).toInstant());
		if (fatherText != null && !fatherText.trim().isEmpty() && motherText != null && !motherText.trim().isEmpty() && selectedDate!=null) {
			btnAddToBrood.setDisable(false);
		} else {
			btnAddToBrood.setDisable(true);
		}
	}

	private ObservableList<Bird> filterBirdsFather(String searchTerm) {
		try {
			ObservableList<Bird> listFathers = birdsRepository.getAllWhereStringAndInteger("Sex", MyValues.MACHO,
					"SpeciesId", CbSpecies.getValue().getId());
			Bird defaultFather = new Bird(null, null, MyValues.SEM_PAI, null, null, null, null, null, null, null, null,
	...
	private ObservableList<Bird> filterBirdsMother(String searchTerm) {
		try {
			ObservableList<Bird> listMothers = birdsRepository.getAllWhereStringAndInteger("Sex", MyValues.FEMEA,
					"SpeciesId", CbSpecies.getValue().getId());
			Bird defaultMother = new Bird(null, null, MyValues.SEM_MAE, null, null, null, null, null, null, null, null,
					null, null, null, null, null, null);
	...
	public void btnAdd(ActionEvent event) throws SQLException {
		if (validator()) {
			...
			String date = new SimpleDateFormat(MyValues.DATE_FORMATE)
					.format(Date.from(DfDataEntrada.getValue().atStartOfDay(ZoneId.systemDefault()).toInstant()));
			...
			if (validateBand(anilha)) {
				...
				if (imageUploaded) {
					...
					try {
						String fileName = anilha
								+ selectedFile.getName().substring(selectedFile.getName().lastIndexOf("."));
						Files.copy(selectedFile.toPath(), defaultFolder.toPath().resolve(fileName),
				...
				bird.setId(BirdId);
				if (selectedBrood != null && bird.getEntryType().equals(MyValues.NASCIMENTO)) {
	...
	public boolean validator() throws SQLException {
		...
		if (validate) {// validate stam in band
			String stam = breederFederationRepository.getStamByBreederAndFederationId(CbCriador.getValue().getId(),
					clubBand.getFederation().getId());
			validate = false;
		...
		if (validate)
			if (TfFather.getText() == null) {
				...
				validate = false;
			} else if (birdsRepository.getBirdWhereString("Band", TfFather.getText()) == null
					&& !TfFather.getText().equals(MyValues.SEM_PAI)) {
				TfFather.setStyle(MyValues.ERROR_BOX_STYLE);
		...
		if (validate)
			if (TfMother.getText() == null) {
				...
				validate = false;
			} else if (birdsRepository.getBirdWhereString("Band", TfMother.getText()) == null
					&& !TfMother.getText().equals(MyValues.SEM_MAE)) {
				TfMother.setStyle(MyValues.ERROR_BOX_STYLE);
	...
	public boolean validateAddtoBrood() {
		boolean validate = false;
		clearAllErrors();
		LabelAlert.setStyle(MyValues.ALERT_ERROR);
		LabelAlert.setText("");
		if (TfFather.getText().equals(MyValues.SEM_PAI)) {
			TfFather.setStyle(MyValues.ERROR_BOX_STYLE);
			LabelAlert.setText("Pai tem de ser escolhido.");
			validate = false;
		} else {
			TfFather.setStyle(null);
			LabelAlert.setText("");
		...
		if (validate) {
			if (TfMother.getText().equals(MyValues.SEM_MAE)) {
				TfMother.setStyle(MyValues.ERROR_BOX_STYLE);
				LabelAlert.setText("Mae tem de ser escolhido.");
				validate = false;
			} else {
				TfMother.setStyle(null);
				LabelAlert.setText("");
	...
	@FXML
	public void btnAddToBrood(ActionEvent event) throws IOException {
		if (validateAddtoBrood()) {
			clearAllErrors();
			LabelAlert.setStyle(null);
			FXMLLoader loader = new FXMLLoader(getClass().getResource("/views/brood/ViewAddBirdToBrood.fxml"));
			Parent root = loader.load();
			ViewAddBirdToBroodController controller = loader.getController();
			Date selectedDate = Date.from(DfDataEntrada.getValue().atStartOfDay(ZoneId.systemDefault()).toInstant());
			controller.setViewAddBirdToBroodController(this, TfFather.getText(), TfMother.getText(),selectedDate);
	...
	public void setEggChose(String msg, Brood brood) throws SQLException {
		LabelAlert.setStyle(MyValues.ALERT_INFO);
		LabelAlert.setText(msg);
		LbBroodChose.setText(brood.toString());
		selectedBrood = brood;
	}
